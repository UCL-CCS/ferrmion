...
pub enum MajoranaEncodingError {
    ...
    TTHartreeFockError(Pauli, Pauli),
    #[error("HF State is not a basis state.")]
    HartreeFockError,
}
...
impl MajoranaEncoding {
            ...
            for (s, &left, &right) in izip!(&mut current_state, &left_ops, &right_ops) {
                ...
                match (left, right) {
                    (Pauli::X | Pauli::Y, Pauli::X | Pauli::Y) => {
                        *s = if s == &true { false } else { true }
                    }
                    (Pauli::Z | Pauli::I, Pauli::Z | Pauli::I) => continue,
    ...
    pub fn hartree_fock_state(
        ...
        for (mode, occ) in fermionic_hf_state.into_iter().enumerate() {
            ...
            Zip::from(&mut current_state)
                ...
                .for_each(|s, &lx, &lz, &rx, &rz| {
                    ...
                    let total_op: PauliMatrix =
                        (left_op - right_op.map(|op| op * Complex64::new(0., 1.)));
        ...
        let hf_components: Array1<bool> = current_state
            .iter()
            .filter_map(|v| {
                if v[0].norm() != 0. && v[1].norm() == 0. {
                    Some(false)
                } else if v[1].norm() != 0. && v[0].norm() == 0. {
                    Some(true)
                } else {
                    println!("{}", v);
                    None
            ...
            .collect();
        println!("HF Components {:?}", &hf_components);
        if hf_components.len() == n_modes {
            Ok(hf_components)
        } else {
            Err(MajoranaEncodingError::HartreeFockError)
        }
    }
}
